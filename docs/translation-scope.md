...
| Global navigation header | Exclude `header[role="banner"]` by default. Translation can be enabled from the popup, but it may interfere with React reconciliation and prevent the global search interface from opening |
...
- `header[role="banner"]` to prevent interference with React reconciliation in the global header. Translation can be enabled from the popup when explicitly desired
