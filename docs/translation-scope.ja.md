...
| グローバルナビゲーションヘッダー | 既定では `header[role="banner"]` を除外。ポップアップで任意に翻訳を有効化できるが、Reactの再描画処理と競合してグローバル検索画面が表示されない可能性がある |
...
- `header[role="banner"]`（グローバルヘッダーのReact再描画処理との競合を防止。ポップアップで任意に翻訳を有効化可能）
